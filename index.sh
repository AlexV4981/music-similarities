...
# Usage:
#   bash index.sh /path/to/Music           # full index
#   bash index.sh /path/to/Music --update  # skip unchanged files
#   bash index.sh /path/to/Music --clean   # remove deleted files from index
# =============================================================================
set -euo pipefail

CYAN='\033[0;36m'; RED='\033[0;31m'; YELLOW='\033[1;33m'; RESET='\033[0m'; BOLD='\033[1m'

if [[ ! -f "backend/app.py" ]]; then
    echo -e "${RED}[✗] Run this from the musemender directory.${RESET}"
    exit 1
fi
...
if [[ $# -lt 1 ]]; then
    echo -e "${RED}[✗] Usage: bash index.sh /path/to/Music [--update|--clean]${RESET}"
    exit 1
fi
